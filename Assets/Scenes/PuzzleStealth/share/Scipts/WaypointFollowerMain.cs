...
public class WaypointFollowerMain : MonoBehaviour
{
    ...
    private void Update()
    {
        ...
        if (guardHealthMain.aliveList[id])
        {
            ...
            else if (guardWait >= 1) // once you've waited (and are still close)
            {
                ...
                guardChaseMain.putWaypoint(startingPosition, id);
        ...
        else if (guardHealthMain.dyingList[id]) // plat our cute little death animation when the guard dies
        {
    ...
    private bool CheckFor(GameObject cf)
    {
        for (float i = 0; i < 50; i++)
        {
            ...
            if (seeingRay.collider != null)
            {
                ...
                {
                    guardChaseMain.putWaypoint(seeingRay.collider.gameObject.transform.position, id);
                    return true;
                }
            }
            if (!movingDown && movingLeft == 180)
