    ...
    private void Update()
    {
        ...
        if (guardHealthT.aliveList[id])
        {
            ...
            else if (guardWait >= 1) // once you've waited (and are still close)
            {
                ...
                guardChaseT.putWaypoint(startingPosition, id);
        ...
        else if (guardHealthT.dyingList[id]) // plat our cute little death animation when the guard dies
        {
    ...
    private bool CheckFor(GameObject cf)
    {
        for (float i = 0; i < 50; i++)
        {
            ...
            if (seeingRay.collider != null)
            {
                ...
                {
                    guardChaseT.putWaypoint(seeingRay.collider.gameObject.transform.position, id);
                    return true;
                }
            }
            if (!movingDown && movingLeft == 180)
