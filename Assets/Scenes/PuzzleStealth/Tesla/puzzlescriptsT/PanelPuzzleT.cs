...
public class PanelPuzzleT : MonoBehaviour
{
    ...
    void Update()
    {
        if (isActivated())
        {
            ...
            if (Input.GetKeyDown(KeyCode.Space))
            {
                ...
                if (x - 1 >= 0)
                {
                    tiles[y * 3 + (x - 1)].GetComponent<panelTileScriptT>().toggleState();
                }
                if (x + 1 <= 2)
                {
                    tiles[y * 3 + (x + 1)].GetComponent<panelTileScriptT>().toggleState();
                ...
                if (y - 1 >= 0)
                {
                    tiles[(y - 1) * 3 + x].GetComponent<panelTileScriptT>().toggleState();
                }
                if (y + 1 <= 2)
                {
                    tiles[(y + 1) * 3 + x].GetComponent<panelTileScriptT>().toggleState();
            ...
            if (allOn())
            {
                objectiveTrackerT.currentObjective.completeObjective(3);
