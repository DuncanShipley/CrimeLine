...
public class bigTextboxT : MonoBehaviour
{
    public bool enabled;
    bool keyDown = false;
    ...
    void Update()
    {
        ...
        if(Input.GetKey(KeyCode.T) && !keyDown)
        {
            ...
            {
                enabled = false;
                GameObject.Find("Main Camera").GetComponent<camfollowT>().enable();
            }
            else
            {
                enabled = true;
                GameObject.Find("Main Camera").GetComponent<camfollowT>().disable();
            }
