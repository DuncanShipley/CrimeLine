...
public class PanelPuzzleTesla : MonoBehaviour
{
    ...
    void Update()
    {
        if (isActivated())
        {
            ...
            {
                tiles[y * 3 + x].GetComponent<panelTileScriptTesla>().toggleState();
                if (x - 1 >= 0)
                {
                    tiles[y * 3 + (x - 1)].GetComponent<panelTileScriptTesla>().toggleState();
                }
                if (x + 1 <= 2)
                {
                    tiles[y * 3 + (x + 1)].GetComponent<panelTileScriptTesla>().toggleState();
                ...
                if (y - 1 >= 0)
                {
                    tiles[(y - 1) * 3 + x].GetComponent<panelTileScriptTesla>().toggleState();
                }
                if (y + 1 <= 2)
                {
                    tiles[(y + 1) * 3 + x].GetComponent<panelTileScriptTesla>().toggleState();
            ...
            if (allOn())
            {
                objectiveTrackerTesla.currentObjective.completeObjective(3);
    ...
    public bool allOn()
    {
        for (int i = 0; i < 9; i++)
        {
            if (!tiles[i].GetComponent<panelTileScriptTesla>().getState())
            {
                return false;
            }
        }
